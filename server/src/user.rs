...
#[get("")]
async fn get_user(req: HttpRequest, state: web::Data<AppState>) -> actix_web::Result<Json<User>, ABError> {
    let auth = req
        .extensions()
        .get::<AuthResponse>()
        .cloned()
        .ok_or(ABError::Unauthorized("Authorization missing or Invalid".to_string()))?;
...
async fn get_oauth_url(
    ...
    if !state.env.enable_google_signin {
        return Err(ABError::BadRequest("Google Sign-in is disabled".to_string()));
    }
