...
use actix_web::{
    get, patch, post, web::{self, Json, Path, Query, ReqData}, HttpResponse, Result, Scope
};
...
async fn create_file(
    ...
    if existing_file.is_some() {
        return Err(ABError::BadRequest(format!("File with file_path '{}' already exists", req.file_path)));
    }
...
async fn get_file(
    ...
    let file = if let Some(f_version) = file_version {
        ...
        return Err(ABError::BadRequest("File key must contain a version or tag".to_string()));
    };
...
async fn upload_bulk_files(
    ...
    let mut archive =
        ZipArchive::new(file).map_err(|e| ABError::BadRequest(e.to_string()))?;
    ...
    if !to_be_uploaded.is_empty() {
        ...
        for mapping in to_be_uploaded.iter().map(|(m, _)| m) {
            ...
            let created_file = inserted_files
                ...
                .ok_or_else(|| ABError::InternalServerError("File not found after insert".to_string()))?;

            let mut entry = archive
                .by_name(&mapping.file_name)
                .map_err(|_| ABError::BadRequest(format!("{} not in archive", mapping.file_name)))?;
