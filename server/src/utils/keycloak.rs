use crate::{
    ...
    types::{ABError, ABResult, AppState, Environment},
};
...
pub async fn find_user_by_username(
    ...
    realm: &str,
    username: &str,
) -> ABResult<Option<UserRepresentation>> {
    let users = admin
        .realm_users_get(
            realm,
            None,
...
pub async fn find_org_group(
    ...
    realm: &str,
    org_name: &str,
) -> ABResult<Option<GroupRepresentation>> {
    let groups = admin
        .realm_groups_get(
            realm,
            None,
        ...
        .await
        .map_err(|_| {
            ABError::InternalServerError(format!("Failed to find group by name: {}", org_name))
        })?;
    ...
    let subgroups = admin
        ...
        .map_err(|_| {
            ABError::InternalServerError(format!(
                "Failed to find subgroups for group: {}",
                group_id
            ))
        })?;
