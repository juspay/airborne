...
pub async fn check_role_hierarchy(
    ...
    let requester_groups = requester_groups_result
        .map_err(|e| OrgAppError::Internal(format!("Failed to get requester groups: {}", e)))?;

    let target_groups = target_groups_result
        .map_err(|e| OrgAppError::Internal(format!("Failed to get target user groups: {}", e)))?;

    let requester_level =
        get_user_highest_level(&requester_groups, app_group_id).ok_or_else(|| {
            OrgAppError::Internal("Failed to determine requester's access level".to_string())
        })?;
...
pub fn validate_access_level(access: &str) -> Result<(String, u8), OrgAppError> {
    match access.to_lowercase().as_str() {
        ...
        _ => Err(OrgAppError::InvalidAccessLevel(format!(
