...
async fn get_release(
    ...
    if release_key.is_empty() {
        return Err(ABError::BadRequest("Release Key cannot be empty".to_string()));
    }

    let auth_response = auth_response.into_inner();
    let organisation =
        validate_user(auth_response.organisation, READ).map_err(|_| ABError::Unauthorized("".to_string()))?;
    let application =
        validate_user(auth_response.application, READ).map_err(|_| ABError::Unauthorized("".to_string()))?;

    let mut conn = state
        .db_pool
        .get()
        .map_err(|_| ABError::InternalServerError("Failed to get database connection".to_string()))?;
...
async fn create_release(
    ...
    let auth_response = auth_response.into_inner();
    let organisation =
        validate_user(auth_response.organisation, WRITE).map_err(|_| ABError::Unauthorized("".to_string()))?;
    let application =
        validate_user(auth_response.application, WRITE).map_err(|_| ABError::Unauthorized("".to_string()))?;
    ...
    let pkg_version = if let Some(package_id) = &req.package_id {
        ...
        if is_first_release {
            return Err(ABError::BadRequest("First release must provide package_id".to_string()));
        }
        if !opt_pkg_version_from_config.is_some() {
            ...
            .first::<i32>(&mut conn)
            .map_err(|_| ABError::NotFound("No packages found for this application".to_string()))?
        }else{
            let version = opt_pkg_version_from_config
                .as_ref()
                .and_then(|v| v.as_i64())
                .map(|v| v as i32)
                .ok_or_else(|| ABError::BadRequest("Could not extract package version from config".to_string()))?;
            version
        }
    };
    ...
    let (final_important, final_lazy, final_resources, final_properties) = if let Some(package_req) = &req.package {
        ...
        if req.package_id.is_some() {
            return Err(ABError::BadRequest("Package ID provided but no package block in request".to_string()));
        }
    ...
    if files.len() != combined_files.len() {
        return Err(ABError::InternalServerError("Some files were missing in DB".to_string()));
    }
    ...
    if let Some(Document::Object(obj)) = &config_document {
        ...
        return Err(ABError::InternalServerError("Resolved config is not an object".to_string()));
    }
...
async fn conclude_release(
    ...
    let experiment_details = state
        ...
        .map_err(|e| {
            eprintln!("Failed to get experiment details: {:?}", e);
            ABError::InternalServerError("Failed to get experiment details from Superposition".to_string())
        })?;
    ...
    let transformed_variant_id = experiment_details
        ...
        .ok_or_else(|| {
            ABError::BadRequest(format!("Variant '{}' not found in experiment. Available variants: {:?}", 
                req.chosen_variant, 
    ...
    state
        ...
        .map_err(|e| {
            ...
            ABError::InternalServerError("Failed to conclude experiment in Superposition".to_string())
        })?;
