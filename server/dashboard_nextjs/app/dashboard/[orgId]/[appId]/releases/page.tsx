...
export default function ReleasesPage() {
  ...
  async function rampRelease(id: string) {
    const input = prompt("Enter new traffic percentage (0-100)", "100")
    if (!input) return
    const pct = Math.min(100, Math.max(0, Number(input)))
    await apiFetch(
  ...
  async function concludeRelease(id: string) {
    if (!confirm("Conclude this release? This will roll out to 100% and finalize.")) return
    await apiFetch(`/releases/${encodeURIComponent(id)}/conclude`, { method: "POST" }, { token, org, app })
    mutate()
  }
