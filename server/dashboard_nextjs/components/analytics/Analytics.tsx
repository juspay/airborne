...
import { fetchAdoptionMetrics, fetchPerformanceMetrics, fetchActiveDevices, calculateMetrics } from "./analyticsUtils";
...
const Analytics: React.FC<AnalyticsProps> = () => {
  ...
  const [adoptionData, setAdoptionData] = React.useState<AdoptionMetrics | null>(null);
  const [performanceData, setPerformanceData] = React.useState<PerformanceMetrics | null>(null);
  const [activeDevicesData, setActiveDevicesData] = React.useState<ActiveDevicesMetrics | null>(null);
  ...
  return (
    <div className="container mx-auto py-6 space-y-8">
      <div className="space-y-4">
        ...
        <div className="text-muted-foreground">View analytics and metrics for your updates and deployments</div>
      </div>
      <Tabs>
        <div className="flex gap-4 items-center p-4">
          ...
          <button
            className={`px-4 py-2 rounded ${
              range.startDate.toDateString() === new Date(today.getTime() - 29 * 24 * 60 * 60 * 1000).toDateString() &&
              range.endDate.toDateString() === today.toDateString()
                ? "bg-primary text-white"
                : "bg-muted"
            }`}
      ...
      <MetricGrid
        metrics={[
          ...
          },
          {
            ...
            value: `${((metrics.updateChecks / (activeDevicesData?.total_active_devices || 1)) * 100).toFixed(1)}%`,
            icon: Clock,
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <AdoptionChart data={adoptionData?.time_breakdown || []} interval={interval} />
        <PerformanceChart data={adoptionData?.time_breakdown || []} interval={interval} />
      </div>

      <div className="grid gap-6 md:grid-cols-2">
        <TimeToAdoptionChart data={adoptionData?.time_breakdown || []} interval={interval} />
        <RollbackChart data={adoptionData?.time_breakdown || []} interval={interval} />
      </div>
    </div>
  );
};
