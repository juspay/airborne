...
export async function fetchAdoptionMetrics(org: string, app: string, interval: "HOUR" | "DAY", range: { startDate: Date; endDate: Date; }) {
  try {
...
export async function fetchPerformanceMetrics(org: string, app: string, interval: "HOUR" | "DAY", range: { startDate: Date; endDate: Date; }) {
  try {
...
export async function fetchActiveDevices(org: string, app: string, interval: "HOUR" | "DAY", range: { startDate: Date; endDate: Date; }) {
  try {
    const diffMs = range.endDate.getTime() - range.startDate.getTime();
    let diffDays = Math.round(diffMs / (1000 * 60 * 60 * 24));
    const params = new URLSearchParams({
...
export function calculateMetrics(adoptionData: AdoptionMetrics | null, performanceData: PerformanceMetrics | null, activeDevicesData: ActiveDevicesMetrics | null) {
  if (!adoptionData) {
    return {
      totalDevices: 0,
      updateChecks: 0,
  ...
  return {
    ...
    successRate:
      (totals.applySuccess / (totals.applySuccess + totals.applyFailures)) * 100 || 0,
    downloadSuccessRate:
      totalDownloads > 0
        ? (totals.downloadSuccess / totalDownloads) * 100
        : 0,
    rollbackRate:
      totals.applySuccess > 0
        ? (totals.rollbacks / totals.applySuccess) * 100
        : 0,
