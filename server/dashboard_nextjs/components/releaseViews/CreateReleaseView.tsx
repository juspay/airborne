...
const CreateReleaseView: React.FC<CreateReleaseViewProps> = ({ onViewCreated }) => {
  ...
  const [dimensionEntries, setDimensionEntries] = useState<DimensionEntry[]>([{ key: "", value: "" }]);
  ...
  const handleEntryChange = (index: number, field: keyof DimensionEntry, value: string) => {
    ...
    <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Please enter the details for the new view.</DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              id="key"
              value={viewName}
              onChange={(e) => setViewName(e.target.value.toLowerCase().replace(/\s+/g, "_"))}
            />
          </div>
            ...
            {dimensionEntries.map((entry, index) => (
              <div key={index} className="flex gap-2 items-center">
                ...
                {dimensionEntries.length > 1 && (
                  <Button type="button" variant="ghost" size="icon" onClick={() => handleRemoveEntry(index)}>
