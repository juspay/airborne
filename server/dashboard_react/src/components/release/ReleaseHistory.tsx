...
import { Calendar, User, Package, Eye, AlertCircle, Loader2, TrendingUp, CheckCircle, Settings } from "lucide-react";
...
const RampModal: React.FC<RampModalProps> = ({ releaseId, organisation, application, release, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    const fetchExperimentDetails = async () => {
      try {
        ...
        if (experimentId) {
          const details = await ReleaseAPI.getExperimentDetails(experimentId, organisation, application);
  ...
  const handleRamp = async () => {
    try {
      setLoading(true);
      await ReleaseAPI.rampRelease(releaseId, {
  ...
  return createPortal(
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[9999] flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <h3 className="text-xl font-bold text-white mb-4">Ramp Up Experiment</h3>
        
        {fetchingCurrent ? (
          <div className="flex items-center justify-center py-8">
            <Loader2 size={24} className="animate-spin text-cyan-400" />
            <span className="ml-2 text-white/80">Loading current traffic...</span>
          </div>
        ) : (
          <div className="space-y-6">
            ...
            <div className="bg-white/5 rounded-lg p-4 border border-white/10">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-cyan-400 font-semibold">{currentTraffic}%</span>
              </div>
              ...
              <div className="flex items-center justify-between mt-3 pt-2 border-t border-white/10">
                ...
                <span className={`px-2 py-1 rounded text-xs font-medium ${
                  experimentStatus === 'CONCLUDED' ? 'bg-green-500/20 text-green-400' :
                  experimentStatus === 'INPROGRESS' ? 'bg-blue-500/20 text-blue-400' :
                  experimentStatus === 'CREATED' ? 'bg-yellow-500/20 text-yellow-400' :
                  experimentStatus === 'DISCARDED' ? 'bg-red-500/20 text-red-400' :
                  experimentStatus === 'PAUSED' ? 'bg-orange-500/20 text-orange-400' :
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-white font-semibold text-lg">{trafficPercentage}%</span>
              </div>
                  ...
                  <input
                    ...
                    style={{
                      background: `linear-gradient(to right, 
                        #0891b2 0%, 
                        #0891b2 ${((trafficPercentage - currentTraffic) / (maxTraffic - currentTraffic)) * 100}%, 
                        rgba(255,255,255,0.1) ${((trafficPercentage - currentTraffic) / (maxTraffic - currentTraffic)) * 100}%, 
              ...
              {experimentStatus === 'CONCLUDED' && (
                <div className="mt-3 p-3 bg-green-500/20 border border-green-500/30 rounded-lg">
                  <p className="text-green-400 text-sm">
                    This experiment has been concluded and cannot be ramped further.
                  </p>
                </div>
              )}
              ...
              {experimentStatus === 'INPROGRESS' && currentTraffic >= maxTraffic && (
                <div className="mt-3 p-3 bg-yellow-500/20 border border-yellow-500/30 rounded-lg">
                  <p className="text-yellow-400 text-sm">
                    This experiment is already at maximum traffic ({maxTraffic}%).
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="flex space-x-3 pt-4">
              ...
              >
                {loading ? <Loader2 size={18} className="animate-spin" /> : "Ramp Up"}
              </button>
            </div>
          </div>
        )}
...
const ConcludeModal: React.FC<ConcludeModalProps> = ({ releaseId, organisation, application, release, onClose, onSuccess }) => {
  ...
  const handleConclude = async () => {
    try {
      setLoading(true);
      await ReleaseAPI.concludeRelease(releaseId, {
  ...
  return createPortal(
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[9999] flex items-center justify-center p-4" onClick={onClose}>
      <div 
        ...
        onClick={(e) => e.stopPropagation()}
      >
        <h3 className="text-xl font-bold text-white mb-4">Conclude Experiment</h3>
        ...
        <div className="flex space-x-3 mt-6">
          ...
          >
            {loading ? <Loader2 size={18} className="animate-spin" /> : "Conclude"}
          </button>
        </div>
      </div>
    </div>,
...
const ReleaseHistory: React.FC<ReleaseHistoryProps> = ({
  ...
  const [rampModalOpen, setRampModalOpen] = useState<ReleaseHistoryEntry | null>(null);
  const [concludeModalOpen, setConcludeModalOpen] = useState<ReleaseHistoryEntry | null>(null);
  const [experimentStatuses, setExperimentStatuses] = useState<Record<string, string>>({});
      ...
      const statusPromises = data.releases
        ...
        .map(async (release: ReleaseHistoryEntry) => {
          try {
            ...
            console.error(`Failed to fetch status for experiment ${release.metadata?.experiment_id}:`, error);
            return { experimentId: release.metadata?.experiment_id, status: 'UNKNOWN' };
          }
          return null;
        });
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-6">
              ...
              <div className="flex items-center space-x-2">
                {hasExperiment(release) && (() => {
                  const experimentId = release.metadata?.experiment_id;
                  const experimentStatus = experimentId ? (experimentStatuses[experimentId] || 'INPROGRESS') : 'INPROGRESS';
                  const isActive = experimentStatus === 'INPROGRESS' || experimentStatus === 'CREATED';
                  ...
                  return (
                    <>
                      ...
                      <div className={`px-2 py-1 rounded text-xs font-medium ${
                        isConcluded ? 'bg-green-500/20 text-green-400' :
