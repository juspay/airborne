CREATE DATABASE IF NOT EXISTS airborneAnalytics;
USE airborneAnalytics;
...
ORDER BY (orgId, appId, eventType, timestamp)
...
ORDER BY (org_id, app_id, event_date, event_hour, release_id)
POPULATE
AS
SELECT 
    orgId as org_id,
...
GROUP BY org_id, app_id, event_date, event_hour, release_id, target_js_version;

-- 2.2 Daily downloads aggregation
CREATE MATERIALIZED VIEW IF NOT EXISTS daily_downloads
ENGINE = SummingMergeTree()
PARTITION BY toYYYYMM(event_date)
ORDER BY (org_id, app_id, event_date, release_id)
POPULATE
AS
SELECT 
    orgId as org_id,
...
FROM ota_events_raw
WHERE eventType = 'DOWNLOAD_COMPLETED'
GROUP BY org_id, app_id, event_date, release_id, target_js_version;
...
PARTITION BY toYYYYMM(event_date)
ORDER BY (org_id, app_id, event_date)
POPULATE
AS
SELECT
    orgId                  AS org_id,
...
ENGINE = SummingMergeTree()
PARTITION BY toYYYYMM(event_date)
ORDER BY (org_id, app_id, event_date, current_js_version)
POPULATE
AS
SELECT
    orgId                               AS org_id,
...
ENGINE = SummingMergeTree()
PARTITION BY toYYYYMM(event_date)
ORDER BY (org_id, app_id, event_date, error_code, event_type)
POPULATE
AS
SELECT
    orgId                             AS org_id,
...
ENGINE = SummingMergeTree()
PARTITION BY toYYYYMM(event_date)
ORDER BY (org_id, app_id, event_date, event_type)
POPULATE
AS
SELECT
    orgId                       AS org_id,
