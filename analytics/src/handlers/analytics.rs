...
use crate::{
    common::{
        ...
        models::{
            ActiveDevicesMetrics, AdoptionMetrics, AnalyticsInterval, FailureMetrics, LoggingInfra,
            PerformanceMetrics, VersionDistribution,
        },
    },
    AppState,
};
    ...
    match params.interval {
        Some(AnalyticsInterval::Day) => {
            if params.start_date.is_none() && params.end_date.is_none() {
                return Err(AppError::Validation(
                    "start_date and end_date in millis must be specified for daywise metrics."
                        .to_string(),
                ));
            }
        }
        Some(AnalyticsInterval::Hour) => {
            if params.date.is_none() {
                return Err(AppError::Validation(
                    "date in millis must be specified for hourly metrics.".to_string(),
                ));
            }
        }
        _ => {
            return Err(AppError::Validation(
                "Interval must be specified. Allowed intervals: DAY, HOUR".to_string(),
            ));
        }
    }

    let metrics = if state.config.logging_infrastructure == LoggingInfra::KafkaClickhouse {
        match state.clickhouse {
            ...
            None => {
                Err(AppError::DatabaseError("Clickhouse client not initialized".to_string()).into())
            }
        }
        ...
        return Err(AppError::Validation(
            "Unsupported logging infrastructure for analytics".to_string(),
        ));
    };
...
pub async fn get_version_distribution(
    ...
    info!(
        "Fetching version distribution for app_id: {} and org_id: {}",
        params.app_id, params.org_id
    );

    let days = params.days.unwrap_or(30);

    let distribution = if state.config.logging_infrastructure == LoggingInfra::KafkaClickhouse {
        match state.clickhouse {
            Some(clickhouse) => {
                clickhouse
                    .get_version_distribution(&params.org_id, &params.app_id, days)
                    .await
            }
            None => {
                Err(AppError::DatabaseError("Clickhouse client not initialized".to_string()).into())
    ...
    match distribution {
        Ok(distribution) => {
            info!(
                "Successfully fetched version distribution for app_id: {} and org_id: {}",
                params.app_id, params.org_id
            );
...
pub async fn get_active_devices(
    ...
    info!(
        "Fetching active devices for org_id: {} and app_id: {}",
        params.org_id, params.app_id
    );
    ...
    let metrics = if state.config.logging_infrastructure == LoggingInfra::KafkaClickhouse {
        match state.clickhouse {
            Some(clickhouse) => {
                clickhouse
                    .get_active_devices_metrics(&params.org_id, &params.app_id, days)
                    .await
            }
            None => {
                Err(AppError::DatabaseError("Clickhouse client not initialized".to_string()).into())
...
pub async fn get_failure_metrics(
    ...
    info!(
        "Fetching failure metrics for org_id: {} and app_id: {}",
        params.org_id, params.app_id
    );
    ...
    let metrics: Result<FailureMetrics, Box<dyn std::error::Error + Send + Sync>> =
        if state.config.logging_infrastructure == LoggingInfra::KafkaClickhouse {
            match state.clickhouse {
                Some(clickhouse) => {
                    let failure_analytics = clickhouse
                    ...
                    let failure_metrics = FailureMetrics {
                        ...
                        failure_rate: if failure_analytics.total_failures > 0 {
                            100.0
                        } else {
                            0.0
                        },
            ...
            match state.victoria {
                Some(victoria) => {
                    let failure_analytics = victoria
                        .get_failure_analytics(
                            &params.org_id,
                            &params.app_id,
                            params.release_id.as_deref(),
    ...
    match metrics {
        Ok(failure_metrics) => {
            info!(
                "Successfully fetched failure metrics for org_id: {} and app_id: {}",
                params.org_id, params.app_id
            );
...
pub async fn get_performance_metrics(
    ...
    info!(
        "Fetching performance metrics for org_id: {} and app_id: {}",
        params.org_id, params.app_id
    );
