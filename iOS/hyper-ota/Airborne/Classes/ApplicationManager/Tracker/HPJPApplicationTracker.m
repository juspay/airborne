...
@interface HPJPApplicationTracker()
...
@implementation HPJPApplicationTracker
...
- (void)trackEventWithLevel:(NSString *)level label:(NSString *)label key:(NSString *)key value:(id)value category:(NSString *)category subcategory:(NSString *)subcategory {
    ...
    for (NSUInteger i = 0; i < [self.loggers count]; i++) {
        ...
        if(logger && [logger conformsToProtocol:@protocol(HPJPLoggerDelegate)]) {
            didSendLog = YES;
    ...
    if(!didSendLog) {
        ...
        if (logsManagerClass) {
            ...
            NSInvocation *invocation = [HPJPHelpers getInvocatorForSelectorString:selectorString className:className isInstanceMethod:NO target:logsManagerClass arguments:@[level, label, finalValue, @"lifecycle", APPL_MANAGER_SUB_CAT, self.workspace]];
            [invocation invoke];
        }
    }
}
