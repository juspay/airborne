...
impl AppConfig {
    pub async fn build(kms_client: &Client) -> Result<Self, String> {
        ...
        let parse_bool = |name: &str, default: bool| -> bool {
            env::var(name)
                .ok()
        ...
        // Helper to parse u16 with default
        let parse_u16 = |name: &str, default: u16| -> u16 {
            env::var(name)
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(default)
        ...
        // Helper to parse usize with default
        ...
        let use_encrypted_secrets: bool = parse_bool("USE_ENCRYPTED_SECRETS", true);
        ...
        let master_key = if use_encrypted_secrets {
            ...
            let key_hex = decrypt_master_key(kms_client, &enc_master_key).await?;
            // Decode hex string to 32-byte key
            let key_bytes = hex::decode(&key_hex)
                .map_err(|e| format!("Failed to decode master key from hex: {}", e))?;
            if key_bytes.len() != 32 {
                return Err(format!(
        ...
        Ok(AppConfig {
            // Server settings
            port: parse_u16("PORT", 8081),
            keep_alive: parse_u64("KEEP_ALIVE", 30),
            backlog: parse_u32("BACKLOG", 1024),
            num_workers: parse_usize("ACTIX_WORKERS", 4),
            ...
            database_pool_size: parse_u32("DATABASE_POOL_SIZE", 4),
            ...
            enable_authenticated_superposition: parse_bool(
                "ENABLE_AUTHENTICATED_SUPERPOSITION",
                false,
            ),

            // Feature flags
            enable_google_signin: parse_bool("ENABLE_GOOGLE_SIGNIN", false),
            organisation_creation_disabled: parse_bool("ORGANISATION_CREATION_DISABLED", false),
