...
async fn update_superposition_build_rid(
    ...
    if dimensions.is_empty() {
        ...
        let r#override = HashMap::from([(
            "build.last_created_build_rid".to_string(),
            Document::String(release_id.to_string()),
        )]);

        let context_request = superposition_sdk::types::ContextPut::builder()
            .set_context(Some(context))
            .set_override(Some(r#override))
            .description(format!(
            ...
            .build()
            .map_err(|e| {
                info!("Failed to construct context request for build rid: {:?}", e);
                ABError::InternalServerError(
                    "Failed to build context request for build.last_created_build_rid".to_string(),
                )
            })?;
