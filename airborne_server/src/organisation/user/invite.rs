...
use crate::{
    middleware::auth::{AuthResponse, WRITE},
    organisation::{
        application::{self, user::find_application},
        user::{
            get_org_context, invite::types::*, transaction::add_user_with_transaction, AccessLvl,
            ApplicationAccess, OrgContext, UserContext,
        },
    },
    run_blocking,
...
pub async fn list_organization_invites(
    ...
    page: u32,
    per_page: u32,
) -> Result<(Vec<OrganisationInviteEntry>, i64), ABError> {
    let pool = pool.clone();
    let org_id = org_id.to_string();
...
pub async fn accept_invite(
    ...
    accept_invitation(admin, realm, &state.db_pool, &request.token, &user_email)
        .await
        .map(Json)
}
...
pub async fn accept_invitation(
    ...
    token: &str,
    user_email: &str,
) -> Result<AcceptInviteResponse, ABError> {
    debug!("Processing invite acceptance for user");
    ...
    info!("Updated invite status to accepted for user");
...
pub async fn validate_invite(
    ...
    state: web::Data<AppState>,
) -> Result<Json<ValidateInviteResponse>, ABError> {
    let request = body.into_inner();
...
pub async fn list_invites(
    ...
    state: web::Data<AppState>,
) -> Result<Json<ListInvitesResponse>, ABError> {
...
pub async fn revoke_invite(
    ...
    state: web::Data<AppState>,
) -> Result<Json<OrganisationInviteEntry>, ABError> {
