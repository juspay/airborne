use crate::{
    ...
    types::{ABError, AppState, Environment, Result},
};
...
pub async fn get_token(env: Environment, client: Client) -> Result<KeycloakAdminToken> {
...
pub async fn find_org_group(
    ...
    realm: &str,
    org_name: &str,
) -> Result<Option<GroupRepresentation>> {
    let groups = admin
        .realm_groups_get(
            realm,
            None,
