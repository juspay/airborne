...
async fn create_token(
    ...
    log::info!("[CREATE TOKEN] Token creation requested");
    ...
    let (org_name, app_name) = match validate_user(auth_response.organisation.clone(), ADMIN) {
        Ok(org_name) => auth_response
            .application
            .ok_or_else(|| {
                log::error!("[CREATE TOKEN] Access denied: no application access");
    ...
    log::info!(
        "[CREATE TOKEN] Validating credentials with Keycloak for org: {}, app: {}",
        org_name,
        app_name
    );
    ...
    log::info!("[CREATE TOKEN] Requesting token from Keycloak");
    ...
    if response.status().is_success() {
        ...
        log::info!(
            "[CREATE TOKEN] Storing credentials in database for org: {}, app: {}",
            org_name,
            app_name
        );
        ...
        log::info!("[CREATE TOKEN] Token created successfully");
...
async fn create_token_oauth(
    ...
    log::info!("[CREATE TOKEN OAUTH] Token creation via OAuth requested");
    ...
    log::info!("[CREATE TOKEN OAUTH] Exchanging authorization code for token");
    ...
    log::info!("[CREATE TOKEN OAUTH] User identity validated");
    ...
    log::info!(
        "[CREATE TOKEN OAUTH] Storing credentials in database for org: {}, app: {}",
        org_name,
        app_name
    );
    ...
    log::info!("[CREATE TOKEN OAUTH] Token created successfully");
...
async fn delete_token(
    ...
    log::info!("[DELETE TOKEN] Token deletion requested");
    let auth_response = auth_response.into_inner();
    let (_organisation, _application) =
        match validate_user(auth_response.organisation.clone(), ADMIN) {
            Ok(org_name) => auth_response
                .application
                .ok_or_else(|| {
                    log::error!("[DELETE TOKEN] Access denied: no application access");
                    ABError::Unauthorized("No Access".to_string())
                })
                .map(|access| (org_name, access.name)),
            Err(_) => {
    ...
    log::info!("[DELETE TOKEN] Token deleted successfully ");
...
async fn list_tokens(
    ...
    log::info!("[LIST TOKENS] Token list requested");
    let auth_response = auth_response.into_inner();
    let (organisation, application) = match validate_user(auth_response.organisation.clone(), ADMIN)
    {
        Ok(org_name) => auth_response
            .application
            .ok_or_else(|| {
                log::error!("[LIST TOKENS] Access denied: no application access");
                ABError::Unauthorized("No Access".to_string())
            })
            .map(|access| (org_name, access.name)),
        Err(_) => validate_user(auth_response.organisation.clone(), READ).and_then(|org_name| {
    ...
    log::info!(
        "[LIST TOKENS] Fetching tokens for org: {}, app: {}",
        organisation,
        application
    );
    ...
    log::info!("[LIST TOKENS] Found {} tokens", tokens.len());
...
async fn issue_token(
    ...
    log::info!("[ISSUE TOKEN] Token issuance requested");
    ...
    log::info!(
        "[ISSUE TOKEN] Decrypting refresh token for org: {}, app: {}",
        user.organisation,
        user.application
    );
    ...
    log::info!("[ISSUE TOKEN] Requesting token from Keycloak using refresh token");
    let response = client.post(&url).form(&params).send().await.map_err(|e| {
    ...
    if response.status().is_success() {
        log::info!("[ISSUE TOKEN] Keycloak authentication successful");
        let token: UserToken = response.json().await.map_err(|e| {
