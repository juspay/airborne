...
If you use the [React Native CLI](/react-native-cli/getting-started), it generates and places `release_config.json` for you.
...
For example, if your namespace is `example`, the path is `app/src/main/assets/example/release_config.json`. The `<namespace>` segment must match the value returned by `getNamespace()` in your [Android setup](/react-native-sdk/integration/android-setup).
...
Once the config is bundled, **[verify the integration](/react-native-sdk/integration/verify)**.
