...
:::note[Prerequisites]
Make sure you have completed [Install the SDK](/react-native-sdk/integration/install) — including adding the Airborne Maven repository — before proceeding.
:::
...
The string returned by `getNamespace()` must match the `<application/namespace-name>` segment of your release URL **and** the folder used for the bundled release config. A mismatch means the SDK cannot find the right bundle. See [Bundling the release config](/react-native-sdk/integration/bundling-release-config).
:::

## Next

Continue with **[iOS setup](/react-native-sdk/integration/ios-setup)**, or jump to [Bundling the release config](/react-native-sdk/integration/bundling-release-config).
