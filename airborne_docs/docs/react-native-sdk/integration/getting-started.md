...
This guide walks through integrating the Airborne SDK into a **plain React Native** app (not Expo — for that, see [Integration in Expo](/react-native-sdk/expo/getting-started)). Because Airborne boots before React Native mounts, the integration happens mostly in your native Android and iOS code.
...
:::note
If you are using the [React Native CLI](/react-native-cli/getting-started), it can scaffold and bundle your `release_config.json` for you. This guide covers both CLI and non-CLI setups.
:::
...
1. **[Install](/react-native-sdk/integration/install)** — add `airborne-react-native` and the Airborne Maven repository.
2. **[Android setup](/react-native-sdk/integration/android-setup)** — wire up `MainActivity` and `MainApplication`.
3. **[iOS setup](/react-native-sdk/integration/ios-setup)** — wire up the bridging header and `AppDelegate`.
4. **[Bundle the release config](/react-native-sdk/integration/bundling-release-config)** — package `release_config.json` for first-boot fallback.
5. **[Verify](/react-native-sdk/integration/verify)** — confirm the SDK boots and serves the OTA bundle.
...
- **Bundled release config path** — where the packaged `release_config.json` lives, used to boot when downloads do not finish within the timeout. See [Bundling the release config](/react-native-sdk/integration/bundling-release-config).
...
- **Tracker** — a stream of SDK events (`onEvent`) you can forward to logging or analytics to monitor release health. See [Callbacks & Events](/react-native-sdk/reference/callbacks-and-events).
- **Lazy Download** — informs the app that specific lazily downloaded files are now available for use.

## Next

Start with **[Install](/react-native-sdk/integration/install)**.
