...
This page wires Airborne into the Android side of an Expo app. The Airborne classes are the same ones used in the [plain React Native track](/react-native-sdk/integration/android-setup); the difference is that they are wrapped in Expo's `ReactActivityDelegateWrapper` and `ReactNativeHostWrapper` so Expo's lifecycle hooks keep running, and the JS main module name points at Expo's virtual metro entry.

:::note[Prerequisites]
Make sure you have installed `airborne-react-native` and run `npx expo prebuild` so the `android/` project exists before proceeding. See [Getting started](/react-native-sdk/expo/getting-started).
:::
...
See the [Android API reference](/react-native-sdk/reference/android-api) for the full `AirborneInterface` surface and the [callbacks & events reference](/react-native-sdk/reference/callbacks-and-events) for the events delivered to `onEvent`.
...
- [Bundle the release config](/react-native-sdk/expo/bundling-release-config) for the first-boot / offline fallback.
- Continue to [iOS setup](/react-native-sdk/expo/ios-setup).
