...
At boot the SDK fetches the [release config](/intro#core-concepts) and starts downloading the current package's files and the release's resources. What the app boots with depends on **which files arrive before the boot timeout**:
...
The boot timeout and the release-config fetch timeout are set per release (`boot_timeout` and `release_config_timeout`). A longer boot timeout lets more files land before boot at the cost of a slower cold start; see [Create a release](/api-reference/endpoints/create-release).
:::
