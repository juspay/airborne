...
Subsequent `airborne-devkit` commands (both the workflow commands and the re-exposed [Core CLI](/core-cli/command-reference) operations) automatically read this token and attach it to each request — you do not pass `--token` yourself.
