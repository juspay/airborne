...
A freshly generated `release_config.json` has this shape, with `url` and `checksum` fields left empty until you run [`create-remote-files`](/react-native-cli/remote-files-and-packages):
