...
Once you have a local `release_config.json` (see [Local configuration](/docs/react-native-cli/local-configuration)), the next two steps push those files to Airborne and bundle them into a deployable package:
...
Both commands accept an optional `[directoryPath]` first argument (defaulting to the current directory), require an existing `airborne-config.json` and `release_config.json`, and require you to have [logged in](/docs/react-native-cli/authentication) first.
...
See the [Core CLI command reference](/docs/core-cli/command-reference#createrelease) for the full parameter set, and the [React Native CLI command reference](/docs/react-native-cli/command-reference) for how the re-exposed commands and input methods work.
