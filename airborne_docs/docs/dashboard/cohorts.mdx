...
A **cohort** segments your users so you can target a whole group with a single value in a [release](/dashboard/releases) rather than spelling out version ranges every time. Cohorts are built on a [cohort dimension](/dashboard/dimensions) (a dimension that *depends on* a standard dimension), and there are two ways to define the segments: **Checkpoints** (version comparisons) and **Groups** (explicit member lists).
...
If you have no cohort dimensions yet, the page prompts you to create one first; choose **Go Back to Dimensions** and create a cohort dimension on the [Dimensions](/dashboard/dimensions) page.
