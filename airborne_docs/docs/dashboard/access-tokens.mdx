...
**Personal access tokens** authenticate the Airborne command-line tools against your application. Each token is a pair — a **client ID** and a **client secret** — that the [React Native CLI](/react-native-cli/authentication) and [Core CLI](/core-cli/authentication) exchange for an access token. Tokens are managed per [application](/dashboard/applications) on the **Tokens** page.
...
- [React Native CLI — Authentication](/react-native-cli/authentication)
- [Core CLI — Authentication](/core-cli/authentication)
