...
Airborne controls access with **role-based access control (RBAC)**. Users are granted **roles**, and each role is a bundle of fine-grained permissions. Access is managed on the **Users** page, which exists at both the [organisation](/dashboard/organisations) level and inside each [application](/dashboard/applications).
