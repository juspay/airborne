...
:::tip
For local development you rarely set these by hand. `make setup` seeds `airborne_server/.env` from `.env.example`, and the init scripts fill in the generated values (Keycloak secrets, Superposition org id, KMS-encrypted master key). See [Run locally](/server/running-locally).
:::
...
`make run` starts a bundled Redis via docker-compose. `REDIS_PORT` (default `6379`) is a **docker-compose-only** variable that maps the container port to the host; the server itself only reads `REDIS_URL`. `make redis` / `make redis-insight` start Redis and the RedisInsight UI on their own. See [Run locally](/server/running-locally).
:::
