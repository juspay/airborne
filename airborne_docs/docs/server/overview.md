...
| **Redis** (optional) | Caches hot reads (files, packages, workspace lookups) and stores the OIDC login PKCE/nonce state so the sign-in flow is hardened. Optional but recommended in production; see [`REDIS_URL`](/server/configuration#redis). |
...
| **KMS** | Key-management service used for envelope encryption of secrets (see [`USE_ENCRYPTED_SECRETS`](/server/configuration#encryption)). LocalStack in dev, real AWS KMS in production. |
| **Analytics server** (optional) | A separate service that ingests OTA events. It runs on one of two stacks: **Kafka + ClickHouse**, or **Victoria Metrics + Grafana**. See the [analytics variables](/server/configuration#analytics-server). |

:::note
Keycloak is the **default** authentication provider, but `AUTHN_PROVIDER` also accepts `oidc`, `okta`, and `auth0`. The Keycloak admin variables (`AUTH_ADMIN_*`) are only mandatory when `AUTHN_PROVIDER=keycloak`. See [Authentication](/server/configuration#authentication--oidc).
:::
...
- [Configuration](/server/configuration) — the complete environment-variable reference. Start here before any deployment.
- [Run locally](/server/running-locally) — bring up the full stack with Docker and `make`.
- [Deploy on AWS ECS](/server/deploy-ecs) and [Deploy on AWS EKS](/server/deploy-eks) — conceptual production guides.
