...
If you have not read it yet, start with the [Configuration](/docs/server/configuration) reference — every environment variable named here is documented there.
...
Set the variables from the [Configuration](/docs/server/configuration) page on the task definition's container. Put **non-secret** values inline (`environment`) and pull **secrets** from AWS Secrets Manager or SSM Parameter Store (`secrets`).
...
- **Encryption** — either `USE_ENCRYPTED_SECRETS=true` with `MASTER_KEY` (the KMS-encrypted DEK) supplied as a secret and a KMS key granted to the task role, **or** `USE_ENCRYPTED_SECRETS=false` with the secret values injected as plaintext from Secrets Manager/SSM. See [Encryption](/docs/server/configuration#encryption).
...
See [boot-time migrations](/docs/server/configuration#boot-time-migrations) for the recognized tokens (`db`, `superposition`, `keycloaktocasbin`).
...
- Task definition env wired per [Configuration](/docs/server/configuration); secrets from Secrets Manager/SSM.
...
For the Kubernetes equivalent, see [Deploy on AWS EKS](/docs/server/deploy-eks).
