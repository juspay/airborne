...
Redirect the user to `auth_url`. When [Redis is configured](/server/configuration#redis), the server stores the PKCE verifier and nonce keyed by the `state` (10-minute, single-use).
...
Create, list, and revoke personal access tokens from the dashboard — see [Access tokens](/dashboard/access-tokens).
