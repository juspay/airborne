...
:::note
Run [`configure --base-url`](/docs/core-cli/getting-started#configure-the-api-base-url) before authenticating, so the CLI knows which Airborne API to talk to.
:::
...
:::note
Unlike the [React Native CLI](/docs/react-native-cli/authentication), the Core CLI does not persist the token to disk for you. `PostLogin` returns the token; capturing and reusing it is up to you. The React Native CLI's `login` command wraps this same operation and stores the result automatically.
:::
...
- [Command reference](/docs/core-cli/command-reference)
