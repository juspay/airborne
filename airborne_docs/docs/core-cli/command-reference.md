...
- Every authenticated operation requires `--token <access-token>` (obtained via `PostLogin` — see [Authentication](/core-cli/authentication)). It is listed in each table for completeness.
- All operations support the three [input methods](/core-cli/getting-started#input-methods): individual flags, a `@params.json` file, or a mix of the two.
- Run `airborne-core-cli configure --base-url <url>` once before using any operation.

:::note
When you invoke these same operations through the [React Native CLI](/react-native-cli/command-reference) (`npx airborne-devkit <Command>`), omit `--token` — `airborne-devkit` injects your stored token automatically.
:::
...
- [Getting started](/core-cli/getting-started)
- [Authentication](/core-cli/authentication)
- [React Native CLI command reference](/react-native-cli/command-reference)
