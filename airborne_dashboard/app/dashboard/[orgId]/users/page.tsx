...
export default function OrganisationUsersPage() {
  const { token, org, getOrgAccess, updateOrgs, organisations } = useAppContext();
  ...
  const handleOrganizationInvite = async (invite: {
    ...
    try {
      // Call the same API as addUser but with applications field matching backend structure
      await apiFetch(
        "/organisations/user/create",
        {
          method: "POST",
          body: {
            user: invite.email,
      ...
      toastSuccess(
        "User Invited",
        `Successfully invited ${invite.email} to the organization with access to ${invite.applications.length} application${invite.applications.length !== 1 ? "s" : ""}`
      );
      ...
      toastError("Failed to Add User", error.message || "Could not add the user to the organization");
    }
  };
