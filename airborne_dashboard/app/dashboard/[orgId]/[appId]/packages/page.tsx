...
const PAGE_AUTHZ = definePagePermissions({
  read_package_group: permission("package_group", "read", "app"),
  create_package_group: permission("package_group", "create", "app"),
...
export default function PackagesPage() {
  ...
  const canCreatePackageGroup = permissions.can("create_package_group");
  const canReadPackages = permissions.can("read_packages");
  const params = useParams<{ appId: string }>();
  const appId = typeof params.appId === "string" ? params.appId : Array.isArray(params.appId) ? params.appId[0] : "";

  useEffect(() => {
    if (permissions.isReady && !permissions.can("read_package_group")) {
      notFound();
    }
