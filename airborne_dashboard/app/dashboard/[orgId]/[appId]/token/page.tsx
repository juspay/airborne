...
const TokensPage = () => {
  ...
  const handleCreateToken = () => {
    if (!passwordLoginEnabled && oidcProviders.length > 0) {
  ...
  const handleOidcLogin = async (idp?: string) => {
    ...
    try {
      const data = await apiFetch<{ auth_url: string; state?: string }>(
        "/users/oauth/url",
        idp ? { query: { offline: true, idp } } : { query: { offline: true } }
      );
      if (data?.auth_url) {
    ...
    <div className="container mx-auto py-8 px-4 max-w-4xl">
      ...
      <Dialog open={showPasswordDialog} onOpenChange={setShowPasswordDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{passwordLoginEnabled ? "Confirm Your Password" : "Continue with Single Sign-On"}</DialogTitle>
            <DialogDescription>
              {passwordLoginEnabled
                ? "Please enter your account password to create a new personal access token."
                : "Use your identity provider account to create a new personal access token."}
            </DialogDescription>
          </DialogHeader>
