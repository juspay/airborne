...
export default function RegisterPage() {
  ...
  const handleOidcSignup = async (idp?: string) => {
    setIsLoading(true);
    try {
      const data = await apiFetch<{ auth_url: string }>("/users/oauth/url", idp ? { query: { idp } } : {});
      if (data?.auth_url) {
        localStorage.setItem("oauthAction", "signup");
        window.location.href = data.auth_url;
      }
    } catch (e: any) {
      console.log("OIDC Sign in Error", e);
      setIsLoading(false);
      // Error toast will be shown automatically by apiFetch
    }
  };
                ...
                <>
                  {oidcProviders.map((provider) => (
                    <Button
                      ...
                      onClick={() => handleOidcSignup(provider.id || undefined)}
                      disabled={isLoading}
                    >
