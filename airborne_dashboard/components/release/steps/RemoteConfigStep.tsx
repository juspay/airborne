...
export function RemoteConfigStep() {
  ...
  const fetchSchema = useCallback(async () => {
    ...
    try {
      ...
      const url = `/organisations/applications/properties/schema`;
      const data = await apiFetch<BackendPropertiesResponse>(url, { method: "GET", headers }, { token, org, app });
      ...
      if (data && data.properties && Object.keys(data.properties).length > 0) {
        ...
        setRemoteConfigValues(buildConfigValuesFromSchema(data.properties, remoteConfigValues));
      }
    } catch (err: any) {
  ...
  const buildPreviewFromSchema = useCallback((): Record<string, any> => {
    return convertDottedToNestedObject(remoteConfigValues);
  }, [remoteConfigValues]);
      ...
      <div key={field.id} className="space-y-2">
        ...
        {field.type === "string" && field.enumValues && field.enumValues.length > 0 ? (
          ...
          <Input
            ...
            type="number"
            value={currentValue ?? ""}
            onChange={(e) => updateRemoteConfigValue(fieldPath, e.target.value ? Number(e.target.value) : undefined)}
          ...
          <div className="space-y-6 min-w-0 overflow-hidden">
            <Textarea
              ...
              rows={3}
              className="font-mono text-sm resize-none w-full break-all"
            />
  ...
  // Calculate highlightedPreview outside JSX
  const highlightedPreview = hljs.highlight(JSON.stringify(buildPreviewFromSchema(), null, 2), {
    language: "json",
  }).value;
          ...
          {schemaLoading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="mt-6 p-4 rounded-lg">
                ...
                <pre className="hljs whitespace-pre-wrap rounded-md p-4 text-xs">
                  <code className="language-json" dangerouslySetInnerHTML={{ __html: highlightedPreview }} />
                </pre>
              </div>
            </div>
          )}
