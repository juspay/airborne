...
export function ReleaseFormProvider({ children, mode, releaseId, initialData }: ReleaseFormProviderProps) {
  ...
  useEffect(() => {
    ...
    const existingPriorities = initialState.filePriority || {};
    ...
    for (const file of selectedPackage.files) {
      ...
      const priority = existingPriorities[fileParsed.filePath] || "important";
      newPriorities[file] = priority;
    }

    setFiles(pkgFiles);
    setFilePriority(newPriorities);
  }, [currentStep, selectedPackage, initialState]);
