...
function ReleaseBuilderContent() {
  ...
  useEffect(() => {
    ...
    apiFetch<any>("/organisations/applications/dimension/list", {}, { token, org, app })
      .then((res) => {
        ...
        const dims = data.map((d: any) => ({
          dimension: d.dimension,
          values: Object.values((d.schema?.properties || {}).value?.enum || d.values || []) as string[],
  ...
  const handleSubmit = async (config: ReleaseConfigRequest) => {
    ...
    try {
      ...
      console.log("Release operation failed", e);
      ...
      <div className="flex items-center justify-between mt-8 pt-6 border-t">
        ...
        <div className="flex gap-2">
          {currentStep < totalSteps ? (
            ...
            <Button onClick={handleCreateClick} disabled={!canProceedToStep(1)}>
              {getActionButtonText()}
            </Button>
          )}
        </div>
