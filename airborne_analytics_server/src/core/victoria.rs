...
use crate::{
    common::{
        models::{
            ...
            DailyActiveDevices, ErrorFrequency, FailureAnalytics, OtaEvent, VersionDistribution,
            VersionMetrics,
        },
        utils,
    },
...
impl Client {
    pub async fn new(victoria_metrics_url: String) -> Result<Self> {
        ...
        let ota_update_available_total = CounterVec::new(
            Opts::new(
                ...
                "Total number of update available events",
            ),
            &["org_id", "app_id", "release_id"],
        )?;
        ...
        let ota_rollback_initiated_total = CounterVec::new(
            Opts::new(
                ...
                "Total number of rollbacks initiated",
            ),
            &["org_id", "app_id", "release_id", "error_code"],
        )?;
        ...
        let ota_rollback_completed_total = CounterVec::new(
            Opts::new(
                ...
                "Total number of rollbacks completed",
            ),
            &["org_id", "app_id", "release_id"],
        )?;
        ...
        let ota_rollback_failures_total = CounterVec::new(
            Opts::new(
                ...
                "Total number of rollback failures",
            ),
            &["org_id", "app_id", "release_id", "error_code"],
        )?;
        ...
        let ota_download_size_bytes = HistogramVec::new(
            HistogramOpts::new("ota_download_size_bytes", "Download size in bytes").buckets(vec![
                1024.0,
                10240.0,
                102400.0,
                1048576.0,
    ...
    pub async fn insert_ota_event(&self, event: &OtaEvent) -> Result<()> {
        ...
        match event.event_type.to_string().as_str() {
            ...
            "DOWNLOAD_FAILED" => {
                ...
                self.ota_download_failures_total
            ...
            "UPDATE_CHECK" => {
                ...
                self.ota_update_checks_total
                    .with_label_values(check_labels)
                    .inc();
            }
            "UPDATE_AVAILABLE" => {
                ...
                self.ota_update_available_total
                    .with_label_values(available_labels)
                    .inc();
            }
            "ROLLBACK_INITIATED" => {
                let rollback_labels = &[
                ...
                self.ota_rollback_initiated_total
                    .with_label_values(rollback_labels)
                    .inc();
            }
            "ROLLBACK_COMPLETED" => {
                let rollback_labels = &[
                ...
                self.ota_rollback_completed_total
                    .with_label_values(rollback_labels)
                    .inc();
            }
            "ROLLBACK_FAILED" => {
                ...
                self.ota_rollback_failures_total
                    .with_label_values(rollback_failure_labels)
                    .inc();
            }
            _ => {
        ...
        if let Some(current_version) = &event.current_js_version {
            ...
            self.ota_device_version
                .with_label_values(version_labels)
                .set(1.0);
        }
    ...
    pub async fn get_active_devices_metrics(
        ...
        let response = self
            .query_client
            .query_range(&query, start_time, end_time, "1d")
            .await?;
    ...
    pub async fn get_failure_analytics(
        ...
        let failure_query = format!(
            ...
            org_id, app_id, release_filter, days, org_id, app_id, release_filter, days
        );
        ...
        let error_query = format!(
            ...
            org_id, app_id, release_filter, org_id, app_id, release_filter
        );
        ...
        let total_failures = failure_response
            .data
            .result
            .first()
        ...
        let total_rollbacks = rollback_response
            .data
            .result
            .first()
            .and_then(|r| r.value.as_ref())
        ...
        let common_errors = common_errors
            .into_iter()
        ...
        while let Some(batch) = futures.next().await {
            ...
            for (ts, count) in data_points {
                let entry = map.entry(ts).or_insert_with(|| AdoptionTimeSeries {
                    time_slot: DateTime::from_timestamp(ts, 0).unwrap_or(Utc::now()),
                    download_success: 0,
                    download_failures: 0,
                    apply_success: 0,
                    apply_failures: 0,
        ...
        while current_time < end_time {
            let _ = map
                ...
                .or_insert_with(|| AdoptionTimeSeries {
                    time_slot: DateTime::from_timestamp(current_time, 0).unwrap_or(Utc::now()),
                    download_success: 0,
                    download_failures: 0,
                    apply_success: 0,
                    apply_failures: 0,
