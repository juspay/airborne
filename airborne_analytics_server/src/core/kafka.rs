...
impl Producer {
    ...
    pub async fn send_ota_event(&self, event: &OtaEvent) -> Result<()> {
        ...
        // Use a combination of org_id, app_id for partitioning
        // This ensures events from the same app go to the same partition
        let key = format!("{}:{}", event.org_id, event.app_id);
...
impl Consumer {
    ...
    pub async fn start_consuming(&self) -> Result<()> {
        ...
        info!(
            "Starting Kafka consumer for OTA events topic: {}",
            self.topic
        );
    ...
    async fn process_event_batch(&self, events: &[OtaEvent]) -> Result<()> {
        ...
        if let Err(e) = self
            .clickhouse
            .insert_ota_events_batch(events.to_vec())
            .await
        {
